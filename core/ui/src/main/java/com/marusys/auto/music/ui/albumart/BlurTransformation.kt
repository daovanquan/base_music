...
class BlurTransformation(
    ...
    override suspend fun transform(
        input: Bitmap,
        size: Size
    ): Bitmap = input.blur(scale, radius, context) ?: input
...
private suspend fun Bitmap.blur(
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S) {
        ...
        val bitmapAlloc = Allocation.createFromBitmap(renderScript, sentBitmap)
        ScriptIntrinsicBlur.create(renderScript, bitmapAlloc.element).apply {
            setRadius(minOf(25f, radius.toFloat()))
            setInput(bitmapAlloc)
            forEach(bitmapAlloc)
        }
        bitmapAlloc.copyTo(sentBitmap)
        renderScript.destroy()
        return@withContext sentBitmap
    }
